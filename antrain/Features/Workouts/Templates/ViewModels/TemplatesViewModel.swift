...
final class TemplatesViewModel {
    ...
    private let trainingProgramRepository: TrainingProgramRepositoryProtocol

    // MARK: - Initialization

    init(
        repository: WorkoutTemplateRepositoryProtocol,
        trainingProgramRepository: TrainingProgramRepositoryProtocol
    ) {
    ...
    /// Check if template is used in any training programs
    /// - Parameter template: Template to check
    /// - Returns: Array of program names using this template, empty if not used
    func checkTemplateUsage(_ template: WorkoutTemplate) async -> [String] {
        do {
            return try await trainingProgramRepository.findProgramsUsingTemplate(template)
        } catch {
            print("❌ Failed to check template usage: \(error)")
            return []
        }
    }
